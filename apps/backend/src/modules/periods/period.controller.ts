...
const periodDatesSchema = z.object({
...
export async function handleCreatePeriod(req: Request, res: Response, next: NextFunction) {
  const parsed = periodDatesSchema.safeParse(req.body);
  if (!parsed.success) {
...
export async function handleUpdatePeriod(req: Request, res: Response, next: NextFunction) {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id <= 0) {
    res.status(400).json({ error: 'period id ไม่ถูกต้อง' });
    return;
  ...
  try {
    const period = await updatePeriod(id, parsed.data, req.lineUser!.companyId);
    res.json(period);
  } catch (err) {
    next(err);
  }
...
export async function handleDeletePeriod(req: Request, res: Response, next: NextFunction) {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id <= 0) {
    res.status(400).json({ error: 'period id ไม่ถูกต้อง' });
    return;
  ...
  try {
    await deletePeriod(id, req.lineUser!.companyId);
    res.status(204).end();
  } catch (err) {
    next(err);
  }
